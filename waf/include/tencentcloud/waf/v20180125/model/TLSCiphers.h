/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef TENCENTCLOUD_WAF_V20180125_MODEL_TLSCIPHERS_H_
#define TENCENTCLOUD_WAF_V20180125_MODEL_TLSCIPHERS_H_

#include <string>
#include <vector>
#include <map>
#include <tencentcloud/core/utils/rapidjson/document.h>
#include <tencentcloud/core/utils/rapidjson/writer.h>
#include <tencentcloud/core/utils/rapidjson/stringbuffer.h>
#include <tencentcloud/core/AbstractModel.h>


namespace TencentCloud
{
    namespace Waf
    {
        namespace V20180125
        {
            namespace Model
            {

                /**
                * TLS 加密套件
                */
                class TLSCiphers : public AbstractModel
                {
                public:
                    TLSCiphers();
                    void ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const;
                    CoreInternalOutcome Deserialize(const rapidjson::Value &value);


                    /**
                     * 获取TLS版本ID
                     * @return VersionId TLS版本ID
                     * 
                     */
                    int64_t GetVersionId() const;

                    /**
                     * 设置TLS版本ID
                     * @param _versionId TLS版本ID
                     * 
                     */
                    void SetVersionId(const int64_t& _versionId);

                    /**
                     * 判断参数 VersionId 是否已赋值
                     * @return VersionId 是否已赋值
                     * 
                     */
                    bool VersionIdHasBeenSet() const;

                    /**
                     * 获取加密套件ID
                     * @return CipherId 加密套件ID
                     * 
                     */
                    int64_t GetCipherId() const;

                    /**
                     * 设置加密套件ID
                     * @param _cipherId 加密套件ID
                     * 
                     */
                    void SetCipherId(const int64_t& _cipherId);

                    /**
                     * 判断参数 CipherId 是否已赋值
                     * @return CipherId 是否已赋值
                     * 
                     */
                    bool CipherIdHasBeenSet() const;

                    /**
                     * 获取加密套件
                     * @return CipherName 加密套件
                     * 
                     */
                    std::string GetCipherName() const;

                    /**
                     * 设置加密套件
                     * @param _cipherName 加密套件
                     * 
                     */
                    void SetCipherName(const std::string& _cipherName);

                    /**
                     * 判断参数 CipherName 是否已赋值
                     * @return CipherName 是否已赋值
                     * 
                     */
                    bool CipherNameHasBeenSet() const;

                private:

                    /**
                     * TLS版本ID
                     */
                    int64_t m_versionId;
                    bool m_versionIdHasBeenSet;

                    /**
                     * 加密套件ID
                     */
                    int64_t m_cipherId;
                    bool m_cipherIdHasBeenSet;

                    /**
                     * 加密套件
                     */
                    std::string m_cipherName;
                    bool m_cipherNameHasBeenSet;

                };
            }
        }
    }
}

#endif // !TENCENTCLOUD_WAF_V20180125_MODEL_TLSCIPHERS_H_
